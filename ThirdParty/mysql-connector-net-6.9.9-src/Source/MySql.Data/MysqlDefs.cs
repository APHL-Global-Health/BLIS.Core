	...
	internal class MySqlConnectAttrs
	{
		...
		public string ProgramName
		{
			get
			{
				...
				try
				{
					var len =  Environment.CommandLine.IndexOf("\" ");
					if(len > -1){
						string path = Environment.CommandLine.Substring(0,len).Trim('"');
		...
		public string Thread
		{
			get
			{
				...
				try
				{
					thread = ((int)AppDomain.GetCurrentThreadId()).ToString();
					var proc = System.Diagnostics.Process.GetCurrentProcess();
					if (proc != null && proc.Threads.Count > 0) {
						var value = proc.Threads [0];
						if (value != null) {
